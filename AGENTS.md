...
- **Not wired yet (needs real frontend work, not just env config)**:
  - **Auth**: there is no login/register UI anywhere in `src/` and `src/services/api.js` has no request interceptor to attach a `Bearer` token — the storefront doesn't need seller auth (that's what `seller-portal/` is for), but it will need its own customer-facing auth eventually for `/account`/`/orders` to be real.
...
- **Not yet done on the backend itself**: order status transitions (`PENDING → PAID → SHIPPED → DELIVERED/CANCELLED` — no route updates `status` yet), linking `Order`/`Product` back to the selling `User` for seller-scoped sales reports (see Reports note below), the admin `/admin` route, and real image upload (currently seed data reuses the mock catalog's external image URLs, and the seller portal's product form only accepts pasted image URLs for the same reason).
